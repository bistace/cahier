...
impl SyntectHighlighter {
    ...
    fn get_syntax_reference(&self) -> &syntect::parsing::SyntaxReference {
        ...
        if shell.ends_with("zsh") {
             syntax_set.find_syntax_by_name("Zsh").unwrap_or_else(|| syntax_set.find_syntax_by_name("Shell-Unix-Generic").unwrap())
        } else if shell.ends_with("bash") {
             syntax_set.find_syntax_by_name("Bash").unwrap_or_else(|| syntax_set.find_syntax_by_name("Shell-Unix-Generic").unwrap())
        } else {
            // Fallback
            syntax_set.find_syntax_by_name("Shell-Unix-Generic")
...
impl Highlighter for SyntectHighlighter {
    fn highlight(&self, line: &str, _cursor: usize) -> StyledText {
        ...
        for (style, text) in highlighter.highlight_line(line, syntax_set).unwrap_or_default() {
